...
$localSecretsDir = Join-Path $root ".local"
$postgresSecretFile = Join-Path $localSecretsDir "postgres-password.secure"
...
function Save-LocalPostgresPassword([string] $value) {
  $secureValue = ConvertTo-SecureString -String $value -AsPlainText -Force
  $secureValue | ConvertFrom-SecureString | Set-Content -LiteralPath $postgresSecretFile -NoNewline
}

function Get-LocalPostgresPassword {
  if (Test-Path $postgresSecretFile) {
    $secureValue = Get-Content -LiteralPath $postgresSecretFile -Raw | ConvertTo-SecureString
    return [System.Net.NetworkCredential]::new('', $secureValue).Password
  }

  if (Test-Path (Join-Path $dataDir "PG_VERSION")) {
    $secureValue = Read-Host "Mavjud lokal PostgreSQL parolini bir marta kiriting" -AsSecureString
    $value = [System.Net.NetworkCredential]::new('', $secureValue).Password
    if ([string]::IsNullOrWhiteSpace($value)) {
      throw "Lokal PostgreSQL paroli kiritilmadi."
    }
    Save-LocalPostgresPassword $value
    return $value
  }
...
New-Item -ItemType Directory -Path $localSecretsDir, $logsDir -Force | Out-Null
$env:PGPASSWORD = Get-LocalPostgresPassword
...
if (-not (Test-Path (Join-Path $dataDir "PG_VERSION"))) {
  Set-Content -LiteralPath $passwordFile -Value $env:PGPASSWORD -NoNewline
...
try {
  ...
  Write-Host "Birinchi administratorni saytning setup oynasi orqali yarating."
