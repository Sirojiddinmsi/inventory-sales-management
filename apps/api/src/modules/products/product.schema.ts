...
export const productImportSchema = z.object({
  rows: z.array(
    z.object({
      ...
      unit: z.string().trim().min(1).max(40).default("dona"),
      purchasePrice: z.coerce.number().min(0).max(9_999_999_999_999_999),
      salePrice: z.coerce.number().min(0).max(9_999_999_999_999_999).default(0),
      quantity: z.coerce.number().min(0).max(999_999_999_999_999),
      minimumStock: z.coerce.number().min(0).max(999_999_999_999_999).default(0),
      location: z.string().trim().max(120).nullish(),
      imageUrl: imagePath.nullish(),
      description: nullableText
    })
